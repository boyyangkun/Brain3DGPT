...
class ConvertToWTOnlyD(MapTransform):
    ...
    def __call__(self, data):
        ...
        seg = d[self.keys[0]]
        # 将所有非零标签设为 1，生成二值掩码
        binary_seg = (seg != 0).astype(seg.dtype)
        d[self.keys[0]] = binary_seg
        return d
...
def custom_transform(patch_shape=96, mode='train',enable_channel_cutmix=False, pair_aug=False, in_channels=4):
    if mode == 'train':
        if enable_channel_cutmix:
            train_transform = transforms.Compose([
                ...
                ConvertToWTOnlyD(keys='label'),  #只保留 WT 单通道
                transforms.EnsureTyped(keys=['image', 'label', 'template'])
    ...
    val_transform = transforms.Compose([
            ...
            # ConvertToMultiChannelBasedOnBratsClassesD(keys='label'), # region-based segmentation
            ConvertToWTOnlyD(keys='label'),  # 👈 只保留 WT 单通道
            #ResizeWithPadOrCropd(keys=['image', 'label'], spatial_size=(155, 240, 240))
            transforms.EnsureTyped(keys=['image', 'label'])
        ])
