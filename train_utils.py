...
def cal_metrics(metrics, acc, loss = 0, current_epoch=0, save_folder=None, mode='validation'):
    ...
    if save_folder:
        ...
        with open(csv_file_path, mode="a", newline='') as f:
            writer = csv.writer(f)
            if not file_exists:
                header = [f'{mode} epoch', 'LOSS', 'ACC', 'AVG'] + [f'class {str(i)}' for i in range(len(metrics))]
                writer.writerow(header)

            row = [current_epoch, loss, acc, np.round(np.mean(metrics), 5)] + [np.round(value, 5) for value in metrics]
            writer.writerow(row)

    return metrics

def cal_save(result, current_epoch=0, save_folder=None, mode='validation'):
    csv_file_path = f"{save_folder}/{mode}_dice.csv"
    file_exists = os.path.exists(csv_file_path)
    with open(csv_file_path, mode="a", newline='') as f:
        writer = csv.writer(f)
        if not file_exists:
            # header = [f'{mode} epoch', 'bleu', 'rouge1', 'meteor', 'bert_f1']
            header = [f'{mode} epoch', 'meteor', 'bert_f1']
            writer.writerow(header)

        row = [current_epoch, result["meteor"], result["bert_f1"]]
        writer.writerow(row)

def save_checkpoint(state: dict, save_folder: Path, best: False):
    if best:
        best_filename = str(save_folder) + '/model_best' + "_" + str(state["epoch"]) + '.pth.tar'
    else:
        best_filename = str(save_folder) + '/model_last' + "_" + str(state["epoch"]) + '.pth.tar'
